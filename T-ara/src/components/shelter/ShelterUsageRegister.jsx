...
const ShelterUsageRegister = () => {
  ...
  const [receiptImage, setReceiptImage] = useState(null); // Base64 이미지 저장용
  ...
  // 사용분야 옵션
  const usageFieldOptions = ["동물", "보호소"];
  ...
  useEffect(() => {
    ...
    //   try {
    //     // 현재 로그인한 보호소 정보 가져오기
    //     const response = await api.get("/shelter/profile");
    //     if (response.data) {
    //       setShelterInfo(response.data);
    //     }
    //   } catch (error) {
    //     console.error("보호소 정보를 가져오는 중 오류가 발생했습니다:", error);
    //   }
    // };
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      if (receipt) {
        ...
        // handleImageUpload 함수 내 OCR 결과 처리 부분 (약 115-145번 줄 근처)
        const items =
          receipt.subResults?.[0]?.items?.map((item) => ({
            name: item.name?.text || item.name?.formatted?.value || "",
            count: (
              item.count?.text ||
              item.count?.formatted?.value ||
              ""
        ...
        setFormData({
          storeName:
            ...
            receipt.storeInfo?.name?.formatted?.value ||
            "",
          businessNumber:
            receipt.storeInfo?.bizNum?.text ||
            receipt.storeInfo?.bizNum?.formatted?.value ||
            "",
          date: formattedDate,
          time:
            receipt.paymentInfo?.time?.text ||
            receipt.paymentInfo?.time?.formatted?.value ||
            "",
          totalAmount: totalAmount,
          paymentMethod:
            receipt.paymentInfo?.cardInfo?.company?.text ||
  ...
  const handleItemChange = (index, field, value) => {
    setFormData((prev) => {
      ...
      if (field === "usageField") {
        ...
        } else if (animalList.find((animal) => animal.animalName === value)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitData = {
        ...
        content: formData.items
          .map((item) => `구매 품목: ${item.name} (${item.count}개)`)
          .join(", "),
        expenseItems: formData.items.map((item) => {
          let relationId = null;

          // usageField에 따라 relationId 설정
          if (item.usageField === "보호소") {
            relationId = null; // 서버에서 인증 토큰으로 자동 처리
          } else if (item.animalId) {
            relationId = item.animalId;
          }
        ...
        receiptImage: receiptImage, // Base64로 인코딩된 이미지 추가
      };

      return api
        .post("/shelter/receipt", submitData)
        .then((response) => {
          console.log("전송 성공:", response);
          alert("영수증이 성공적으로 등록되었습니다.");
          navigate("/shelter/usage");
        })
        .catch((error) => {
  ...
  // 사용분야 옵션 생성
  const getUsageFieldOptions = () => {
    const options = ["보호소"];

    if (animalList && animalList.length > 0) {
      animalList.forEach((animal) => {
        options.push(animal.animalName);
      });
    }
  ...
  const renderForm = () => {
    const extendedUsageOptions = getUsageFieldOptions();
              ...
              <div key={index} className="grid grid-cols-19 gap-3 items-start">
                ...
                <div className="col-span-3">
                  <select
                    ...
                    onChange={(e) =>
                      handleItemChange(index, "usageField", e.target.value)
                    }
                    className="w-full p-2 border rounded"
                  >
                    <option value="">사용분야 선택</option>
                    {extendedUsageOptions.map((option) => (
                      <option key={option} value={option}>
                        {option}
                      </option>
                    ))}
